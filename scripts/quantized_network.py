...
class QuantizedNeuralNetwork:
    def __init__(
        ...
        self.residuals = {
            layer_idx: zeros(
                (
                    ...
                    int(
                        dims[0] / order
                    ),  # N_{ell} dimensional feature, order bits per iteration.
                    self.batch_size,
                )
            )  # Dimension of the residual vectors.
        ...
        self.layerwise_rel_errs = {
            layer_idx: zeros(dims[0],)  # N_{ell} dimensional feature,
            for layer_idx, dims in self.layer_dims.items()
        }
    ...
    def quantize_layer(self, layer_idx: int, order: int):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if order == 1:
                ...
                future_to_neuron = {
                    executor.submit(
                        self.quantize_neuron2, layer_idx, neuron_idx, wX, qX, rad
                    ): neuron_idx
                    for neuron_idx in range(N_ell_plus_1)
                }
    ...
    def neuron_dashboard(self, layer_idx: int, neuron_idx: int) -> Axes:
        ...
        fig.suptitle(
            f"Dashboard for Neuron {neuron_idx} in Layer {layer_idx}", fontsize=24
        )
        ...
        resid_ax.plot(
            range(N_ell),
            [norm(self.residuals[layer_idx][neuron_idx, t, :]) for t in range(N_ell)],
            "-o",
        )
    ...
    def layer_dashboard(self, layer_idx: int) -> Axes:
        ...
        fig, axes = subplots(2, 2, figsize=(10, 10))
        fig.suptitle(
            f"Dashboard for Layer {layer_idx}: $(N_{{L}}, N_{{L+1}}, d)$ = ({N_ell}, {N_ell_plus_1}, {self.batch_size})"
        )
        ...
        sup_resids = [
            max([norm(U_tensor[neuron_idx, t, :]) for neuron_idx in range(N_ell_plus_1)])
