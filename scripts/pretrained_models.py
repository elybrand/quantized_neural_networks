...
# Here are all the parameters we iterate over for pretrained models.
trial_idxs = [0]
q_train_sizes = [10**4]
ignore_layers = [[]]
...
ParamConfig = namedtuple('ParamConfig', 'data_set, trial_idx, np_seed, tf_seed, q_train_size, ignore_layer')
...
def train_network(parameters: ParamConfig) -> pd.DataFrame:
	...
	logger.info(f"Training with parameters {parameters}")
	...
	model = Sequential()
	model.add(Conv2D(filters=64, kernel_size=parameters.kernel_size, strides=(parameters.stride, parameters.stride), activation=parameters.rectifier, kernel_initializer=parameters.kernel_init(), input_shape=input_shape))
	model.add(BatchNormalization())
	model.add(Conv2D(filters=32, kernel_size=parameters.kernel_size,  strides=(parameters.stride, parameters.stride), kernel_initializer=parameters.kernel_init(), activation=parameters.rectifier))
	model.add(BatchNormalization())
	model.add(Flatten())
	...
	history = model.fit(X_net_train, y_net_train, batch_size=parameters.train_batch_size, epochs=parameters.epochs, verbose=True, validation_split=.20)
