...
history = model.fit(
    X_train, y_train, batch_size=128, epochs=100, verbose=True, validation_split=0.10,
)
...
for i in range(num_layers - 1):
    ...
    get_data2 = chain(get_data2, (sample for sample in X_train))
...
axes[0].legend(["MSQ", r"$\Sigma\Delta$MPSort", r"$\Sigma\Delta$"], fontsize=12)
...
axes[1].legend(["MSQ", r"$\Sigma\Delta$MPSort", r"$\Sigma\Delta$"], fontsize=12)
