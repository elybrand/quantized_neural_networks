...
Everything else about serializing the quantized models and so forth is the same as in the MNIST & CIFAR10 sections. The only difference is that since we're quantizing only one network per process, the model test accuracies are logged in `.csv` files with only one row. That's super annoying to compare to other hyperparameter configurations. So, I wrote yet another script to collect all files in the `model_metrics/` subdirectory which are generated from the ImageNet script and groups them into one `.csv` file. Once you've got a couple of `.csv` files from quantizing ImageNet in the `model_metrics/` directory, you can combine them into a new `.csv` file by running `python combine_metrics_script.py`. You're welcome.
